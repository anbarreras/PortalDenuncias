		...
		do {
			...
			if (inicio.equalsIgnoreCase("si")) {
				...
				if (respuesta.equals("consultar")) {
					...
					pu.setCedula(lector.nextLine());

					for (i = 0; i < max_usuarios; i++) {
						if (pu.getCedula(pu.cedula).equals(pArray[i].cedula)) { // PArray[i].getCedula(p.cedula)
					...
					if (respuesta2.equalsIgnoreCase("SI")) {
						...
						if (respuesta3.equalsIgnoreCase("edad")) {
							...
							System.out.println("Ingrese la correción del dato(fecha en formato DD/MM/AAAA)");
							pu.setFecha(lector.nextLine());
