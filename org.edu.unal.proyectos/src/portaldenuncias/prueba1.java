		...
		while (true) {
			...
			if (respuesta.equals("consultar")) {
				...
				for(i = 0; i < max_personas;i++) {
					if(cedula.equals(usuario[i][0])) {
						System.out.println(
								" la denuncia hecha fue realizada por:\"" + nombre + "\" su cedula es: \"" + usuario[i][0]
										+ "\" su edad: \"" + usuario[i][1] + "\" la ciudad en que sucedio el problema:\""
										+ usuario[i][2] + "\" y lo que sucedio fue: \"" + usuario[i][3] + "\" ");
						break;
					}
			...
			if (respuesta.equals("denunciar")) {
				...
				while (persona < max_personas) {
					...
					System.out.println(
							"Si estos son los datos correctos escriba SI (en mayusculas y sin espacios) o NO (sin mayusculas y sin espacios) si hay algun error (tenga en cuenta que si dice \"NO\" debera rellenar el formulario completo nuevamente");
